  ...
  return (
    <Link
      ...
      className={`bg-white rounded-xl shadow-xl w-full ${
        isSm ? 'max-w-[280px] pt-4 pb-5 pl-4 pr-4' : 'max-w-xs pt-5 pb-7 pl-5 pr-5'
      }`}
    >
      ...
      <h2 className={`${isSm ? 'pt-3 pb-2 text-base' : 'pt-4 pb-2 text-lg'} font-semibold text-text-darker`}>
        {blogTitle}
      </h2>
      ...
      <div className={`flex flex-col justify-center ${isSm ? 'pt-2' : 'pt-2'}`}>
        <div className="flex items-center gap-3">
          <img
            src={userImg}
            alt={user}
            className={`${isSm ? 'w-9 h-9' : 'w-10 h-10'} rounded-full object-cover`}
          />
          <div className="flex flex-col flex-1">
            <p className="text-xs text-gray-600">written by</p>
            <div className="flex justify-between w-full">
              <p className={`${isSm ? 'text-sm' : 'text-base'} font-semibold text-black`}>{user}</p>
